...
export async function GET(request: NextRequest) {
  try {
    ...
    const settlementStatus = searchParams.get("settlementStatus") || "pending"; // pending, paid, all
    ...
    // Filter by settlement status - handle nullable boolean properly
    if (settlementStatus === "pending") {
      whereClause.OR = [
        { isSettlement: false },
        { isSettlement: null }  // Handle nullable boolean (default)
      ];
    } else if (settlementStatus === "paid") {
      whereClause.isSettlement = true;
    }
    // If "all", don't add settlement filter
    
    console.log("Settlement status filter:", settlementStatus);
    ...
    const settlementData = orderItems.map((item) => {
      ...
      const productTotal = Number(item.priceAtPurchase);
      ...
      const commissionAmount = (productTotal * Number(commissionPercentage)) / 100;
      
      // Calculate amount after commission deduction
      const amountAfterCommission = productTotal - commissionAmount;
      
      // Calculate GST on the amount after commission
      // GST should be calculated on: (productTotal - commissionAmount)
      // Get GST percentage dynamically from database - NO HARDCODING - same as invoice route
      let gstPercentage = 0;
      ...
      if (gstPercentage === 0 && item.gstAmountAtPurchase) {
        ...
        if (storedGstAmount > 0) {
          gstPercentage = (storedGstAmount / productTotal) * 100;
      ...
      // Product total for display (base price + GST on base price) - same as invoice route
      const displayProductTotal = productTotal + Number(item.gstAmountAtPurchase || 0);
      
      // Debug logging to understand the calculation
      console.log(`Settlement Calculation for ${item.id}:`, {
        productTotal,
        displayProductTotal,
      ...
      return {
        ...
        productTotal: displayProductTotal, // Use the display total (base + GST)
        discountAmount,
        gstAmount,
        shippingCharge,
